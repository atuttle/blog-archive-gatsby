...
The next relevant information about Application.cfc is add some configuration in [variables.framework](http://docs.taffy.io/3.2.0#variablesframework-settings). That link shows the default configuration, so you only need to include/change bits that you want to be different. Each setting is loaded/defaulted individually so that you can use the object shorthand syntax `variables.framework = { ... };` to specify only the settings you want to override, and the rest will still use their defaults.
...
><img src="https://restassuredbook.com/assets/img/book-3d.png" alt="Cover art for REST Assured" class="hero" width="100" style="float:right; margin-bottom:0" />
